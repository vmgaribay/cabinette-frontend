...
const [messageQueue, setMessageQueue] = useState<{ id: number; text: string }[]>([]);
...
function handleDownload() {
  ...
  const blob = new Blob([JSON.stringify(metaObject,null,2)], { type: "application/json" });
  ...
  reader.onload = (e) => {
    try {
      ...
      if (validityCheck(imported)) {
        ...
        addMessage(`Bookmarks from file created on ${new Date(imported.date).toLocaleString(undefined, { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })} loaded successfully.`);
      } else {
        alert("Invalid bookmarks file. Please select a file exported from this app.");
      }
    } catch {
      alert("Failed to parse bookmarks file. Please select a file exported from this app.");
    }
  };
...
function handleAddToExisting(event: React.ChangeEvent<HTMLInputElement>) {
  ...
  reader.onload = (e) => {
    try {
      const imported = JSON.parse(e.target?.result as string);
      if (validityCheck(imported)) {
        const merged = Array.from(new Set([...bookmarks, ...imported.bookmarks]));
        dispatch(setBookmarks(merged));
        addMessage(`Bookmarks merged with file created on ${new Date(imported.date).toLocaleString(undefined, { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' })}.`);
        handleDownload();
      } else {
        alert("Invalid bookmarks file. Please select a file exported from this app.");
      }
 ...
 */
function validityCheck(sample: any): sample is { date: string; bookmarks: string[] } {
  return (
    sample &&
    typeof sample === "object" &&
    typeof sample.date === "string" &&
    Array.isArray(sample.bookmarks) &&
    sample.bookmarks.every((id: any) => typeof id === "string")
  );
}
...
<button onClick={() => setMenuOpen((open) => !open)} title="Save or load existing bookmarks"
            style={{
              fontSize: 14,
          ...
          > <HiArrowDownTray />/<HiArrowUpTray /> <HiOutlineBookmark /> Utilities</button> 
          {menuOpen && (
  <div
    ref={menuRef}
    style={{
  ...
  >
    <button className="menu-item" title="Export/download current bookmarks as a JSON file." onClick={handleDownload}><HiArrowDownTray /> Export Bookmarks</button>
    <label className="menu-item" title="Import/upload bookmarks from a specified file." style={{ cursor: "pointer" }}><HiArrowUpTray /> Import Bookmarks<input type="file" accept="application/json" style={{ display: "none" }} onChange={handleRestore} /></label>
    <label className="menu-item" title="Retrieve bookmarks from a specified file, merge with current bookmarks, and export merged bookmarks to a new file." style={{ cursor: "pointer" }}><input type="file" accept="application/json" style={{ display: "none" }} onChange={handleAddToExisting} />Add to Existing Bookmarks</label>
    <button className="menu-item" title="Close menu." onClick={() => setMenuOpen(false)}>X Cancel</button>
  </div>
